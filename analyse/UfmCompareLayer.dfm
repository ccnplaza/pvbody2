object fmCompareLayer: TfmCompareLayer
  Left = 0
  Top = 0
  Caption = #48708#44368#48516#49437'('#47112#51060#50612')'
  ClientHeight = 687
  ClientWidth = 1078
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = #44404#47548
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Visible = True
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnlMember: TPanel
    Left = 0
    Top = 0
    Width = 210
    Height = 687
    Align = alLeft
    BevelOuter = bvNone
    TabOrder = 0
    inline frmMemberSelect1: TfrmMemberSelect
      Left = 0
      Top = 0
      Width = 210
      Height = 687
      Align = alClient
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = #44404#47548
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      ExplicitWidth = 210
      ExplicitHeight = 687
      inherited cxGroupBox1: TcxGroupBox
        ExplicitWidth = 208
        ExplicitHeight = 584
        Height = 584
        Width = 208
        inherited cxGrid1: TcxGrid
          Width = 202
          Height = 534
          ExplicitWidth = 202
          ExplicitHeight = 534
          inherited gridCheck: TcxGridDBTableView
            OnCellClick = frmMemberSelect1gridCheckCellClick
          end
        end
        inherited pnlPictureButton: TPanel
          Width = 202
          ExplicitWidth = 202
        end
      end
      inherited cxGroupBox2: TcxGroupBox
        ExplicitWidth = 208
        Width = 208
      end
    end
  end
  object Panel1: TPanel
    Left = 808
    Top = 0
    Width = 270
    Height = 687
    Align = alRight
    TabOrder = 1
    object Panel2: TPanel
      Left = 1
      Top = 276
      Width = 268
      Height = 410
      Align = alClient
      BevelOuter = bvNone
      TabOrder = 0
      object cxGrid2: TcxGrid
        Left = 0
        Top = 0
        Width = 268
        Height = 410
        Align = alClient
        PopupMenu = PopupLayer
        TabOrder = 0
        LookAndFeel.SkinName = 'Black'
        object gridCompare: TcxGridDBTableView
          Navigator.Buttons.CustomButtons = <>
          Navigator.Buttons.First.Visible = True
          Navigator.Buttons.PriorPage.Visible = True
          Navigator.Buttons.Prior.Visible = True
          Navigator.Buttons.Next.Visible = True
          Navigator.Buttons.NextPage.Visible = True
          Navigator.Buttons.Last.Visible = True
          Navigator.Buttons.Insert.Visible = True
          Navigator.Buttons.Append.Visible = False
          Navigator.Buttons.Delete.Visible = True
          Navigator.Buttons.Edit.Visible = True
          Navigator.Buttons.Post.Visible = True
          Navigator.Buttons.Cancel.Visible = True
          Navigator.Buttons.Refresh.Visible = True
          Navigator.Buttons.SaveBookmark.Visible = True
          Navigator.Buttons.GotoBookmark.Visible = True
          Navigator.Buttons.Filter.Visible = True
          OnCellDblClick = gridCompareCellDblClick
          DataController.DataSource = d_compare_layer
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <>
          DataController.Summary.SummaryGroups = <>
          OptionsSelection.CellSelect = False
          OptionsSelection.HideFocusRectOnExit = False
          OptionsSelection.UnselectFocusedRecordOnExit = False
          OptionsView.ColumnAutoWidth = True
          OptionsView.GroupByBox = False
          OptionsView.Indicator = True
          object gridCompareID: TcxGridDBColumn
            DataBinding.FieldName = 'ID'
            Visible = False
          end
          object gridCompareMEMBER_UID: TcxGridDBColumn
            DataBinding.FieldName = 'MEMBER_UID'
            Visible = False
          end
          object gridCompareMAKE_DATE: TcxGridDBColumn
            Caption = #45216#51676
            DataBinding.FieldName = 'MAKE_DATE'
            Options.Editing = False
            Width = 87
          end
          object gridCompareCOMPARE_NAME: TcxGridDBColumn
            Caption = #51228#47785
            DataBinding.FieldName = 'COMPARE_NAME'
            Width = 163
          end
        end
        object cxL2: TcxGridLevel
          GridView = gridCompare
        end
      end
    end
    object Panel3: TPanel
      Left = 1
      Top = 1
      Width = 268
      Height = 275
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 1
      object GroupBox1: TGroupBox
        Left = 5
        Top = 102
        Width = 260
        Height = 49
        Caption = #47112#51060#50612#53080#53944#47204
        TabOrder = 0
        object btnDeleteLayer: TBitBtn
          Left = 112
          Top = 16
          Width = 35
          Height = 25
          Action = ActDeleteSelect
          Glyph.Data = {
            
            F30A0F0000000000030D0FF3F3F3F3F3F80AA0000000000000AD}
          ParentShowHint = False
          ShowHint = True
          TabOrder = 0
        end
        object btnUp: TBitBtn
          Left = 180
          Top = 16
          Width = 31
          Height = 25
          Action = ActLayerToUp
          Glyph.Data = {
            
            DADAADADADA0ADADADADDADADADADADADADAADADADADADADADAD}
          ParentShowHint = False
          ShowHint = True
          TabOrder = 1
        end
        object btnDown: TBitBtn
          Left = 210
          Top = 16
          Width = 30
          Height = 25
          Action = ActLayerToDown
          Glyph.Data = {
            
            DADAADADA00000ADADADDADADADADADADADAADADADADADADADAD}
          ParentShowHint = False
          ShowHint = True
          TabOrder = 2
        end
        object chkRotateLayer: TCheckBox
          Left = 11
          Top = 19
          Width = 97
          Height = 17
          Action = ActRotationMode
          TabOrder = 3
        end
        object btnDeleteLayerAll: TBitBtn
          Left = 146
          Top = 16
          Width = 34
          Height = 25
          Action = ActDeleteAll
          Glyph.Data = {
            
            0ADAA00DADADADAD00ADDADADADADADADADAADADADADADADADAD}
          ParentShowHint = False
          ShowHint = True
          TabOrder = 4
        end
      end
      object GroupBox2: TGroupBox
        Left = 5
        Top = 154
        Width = 260
        Height = 92
        Caption = #47112#51060#50612#54952#44284
        TabOrder = 1
        object Label8: TLabel
          Left = 16
          Top = 63
          Width = 39
          Height = 13
          Caption = #53804#47749#46020
        end
        object Label9: TLabel
          Left = 16
          Top = 40
          Width = 95
          Height = 13
          Caption = #47112#51060#50612' '#54633#49457#47784#46300
        end
        object chkLayerVisible: TCheckBox
          Left = 166
          Top = 17
          Width = 65
          Height = 17
          Action = ActEffectLayer
          State = cbChecked
          TabOrder = 0
        end
        object cboLayerOperation: TComboBox
          Left = 133
          Top = 37
          Width = 119
          Height = 21
          ImeName = 'Microsoft IME 2010'
          ItemIndex = 0
          TabOrder = 1
          Text = 'Normal'
          OnChange = cboLayerOperationChange
          Items.Strings = (
            'Normal'
            'Add (Additive)'
            'Sub (Difference)'
            'Div'
            'Mul'
            'OR'
            'AND'
            'XOR'
            'MAX (Lighten)'
            'MIN (Darken)'
            'Average'
            'Screen'
            'Negation'
            'Exclusion'
            'Overlay'
            'HardLight'
            'SoftLight'
            'XFader'
            'ColorEdge'
            'ColorBurn'
            'InverseColorDodge'
            'InverseColorBurn'
            'SoftDodge'
            'SoftBurn'
            'Reflect'
            'Glow'
            'Freeze'
            'Eat'
            'Subtractive'
            'Interpolation'
            'Stamp'
            'Red'
            'Green')
        end
        object chkLayerAutoFit: TCheckBox
          Left = 16
          Top = 17
          Width = 73
          Height = 17
          Action = ActLayerAutofit
          State = cbChecked
          TabOrder = 2
        end
        object chkDrawBox: TCheckBox
          Left = 95
          Top = 17
          Width = 65
          Height = 17
          Action = ActLayerOutline
          State = cbChecked
          TabOrder = 3
        end
        object edtTransperency: TcxSpinEdit
          Left = 204
          Top = 59
          Properties.AssignedValues.DisplayFormat = True
          Properties.AssignedValues.MinValue = True
          Properties.ImmediatePost = True
          Properties.MaxValue = 255.000000000000000000
          Properties.UseCtrlIncrement = True
          Properties.UseDisplayFormatWhenEditing = True
          Properties.ValueType = vtInt
          Properties.OnEditValueChanged = edtTransperencyPropertiesEditValueChanged
          Style.LookAndFeel.NativeStyle = True
          StyleDisabled.LookAndFeel.NativeStyle = True
          StyleFocused.LookAndFeel.NativeStyle = True
          StyleHot.LookAndFeel.NativeStyle = True
          TabOrder = 4
          Value = 128
          Width = 51
        end
        object TrackBar1: TTrackBar
          Left = 63
          Top = 59
          Width = 141
          Height = 20
          Max = 255
          Position = 128
          TabOrder = 5
          OnChange = TrackBar1Change
        end
      end
      object imgMThumbView: TImageEnMView
        AlignWithMargins = True
        Left = 3
        Top = 3
        Width = 262
        Height = 93
        Background = clGray
        ScrollBars = ssNone
        GridWidth = 20
        StoreType = ietThumb
        ThumbWidth = 40
        ThumbHeight = 80
        TextMargin = 0
        ThumbnailOptionsEx = [ietxShowIconForUnknownFormat, ietxShowIconWhileLoading, ietxEnableInternalIcons, ietxStretchSmallImages, ietxAutoPadThumbnailColumns, ietxCenterSingleFrame]
        PopupMenus = []
        OnImageSelect = imgMThumbViewImageSelect
        MouseInteract = [mmiScroll, mmiSelect]
        SelectionWidth = 3
        SelectionColor = clRed
        Style = iemsFlat
        AutoAdjustStyle = False
        ThumbnailsBackground = clBlack
        ThumbnailsBackgroundSelected = clBtnFace
        ThumbnailsBorderWidth = 0
        ThumbnailsBorderColor = clLime
        ThumbnailsInternalBorderColor = clLime
        MultiSelectionOptions = []
        DefaultTopText = iedtNone
        DefaultInfoText = iedtNone
        DefaultBottomText = iedtNone
        Align = alTop
        TabOrder = 2
        EnableMultiSelect = True
      end
      object btnSaveSetting: TBitBtn
        Left = 138
        Top = 247
        Width = 121
        Height = 27
        Action = ActSaveSetup
        Caption = #49444#51221#44050#51200#51109
        Glyph.Data = {
          
          30DAAD000DAD003330ADDADADA0033000ADAADADADA0000DADAD}
        TabOrder = 3
      end
    end
  end
  object ImageEnViewCompare: TImageEnView
    Left = 346
    Top = 0
    Width = 462
    Height = 687
    Background = clInactiveCaptionText
    BorderStyle = bsNone
    Align = alClient
    PopupMenu = PopupMenu1
    TabOrder = 2
  end
  object pnlProcess: TPanel
    Left = 416
    Top = 167
    Width = 217
    Height = 49
    BevelOuter = bvLowered
    BorderStyle = bsSingle
    Color = clBlue
    DoubleBuffered = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = #44404#47548
    Font.Style = [fsBold]
    ParentBackground = False
    ParentDoubleBuffered = False
    ParentFont = False
    TabOrder = 3
    Visible = False
    object lblCount: TLabel
      AlignWithMargins = True
      Left = 4
      Top = 27
      Width = 205
      Height = 13
      Margins.Top = 0
      Align = alTop
      Alignment = taCenter
      AutoSize = False
      Caption = '0/10'
      EllipsisPosition = epEndEllipsis
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = #44404#47548
      Font.Style = [fsBold]
      ParentFont = False
      ExplicitLeft = 0
      ExplicitTop = 14
      ExplicitWidth = 217
    end
    object lblMessage: TLabel
      AlignWithMargins = True
      Left = 4
      Top = 11
      Width = 205
      Height = 13
      Margins.Top = 10
      Align = alTop
      Alignment = taCenter
      AutoSize = False
      Caption = #51089#50629#51473'...'
      EllipsisPosition = epEndEllipsis
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = #44404#47548
      Font.Style = [fsBold]
      ParentFont = False
      ExplicitLeft = 36
      ExplicitTop = 31
    end
  end
  object Panel4: TPanel
    Left = 210
    Top = 0
    Width = 136
    Height = 687
    Align = alLeft
    BevelOuter = bvNone
    Caption = 'Panel3'
    TabOrder = 4
    object cxGroupBox3: TcxGroupBox
      AlignWithMargins = True
      Left = 1
      Top = 1
      Margins.Left = 1
      Margins.Top = 1
      Margins.Right = 1
      Margins.Bottom = 1
      Align = alClient
      Caption = #51088#49464#49324#51652
      TabOrder = 0
      Height = 685
      Width = 134
      object ImageEnMView: TImageEnMView
        Left = 3
        Top = 15
        Width = 128
        Height = 660
        Cursor = crHandPoint
        Margins.Left = 1
        Margins.Top = 1
        Margins.Right = 1
        Margins.Bottom = 1
        Background = clGray
        ScrollBars = ssVertical
        BorderStyle = bsNone
        GridWidth = 1
        StoreType = ietNormal
        ThumbWidth = 102
        ThumbHeight = 250
        HorizBorder = 2
        VertBorder = 2
        BottomGap = 2
        UpperGap = 2
        LeftGap = 0
        TextMargin = 0
        ThumbnailOptionsEx = [ietxShowIconForUnknownFormat, ietxShowIconWhileLoading, ietxEnableInternalIcons, ietxStretchSmallImages, ietxFillWithImgBackground, ietxAutoPadThumbnailColumns, ietxCenterSingleFrame]
        PopupMenus = []
        MouseInteract = [mmiScroll, mmiSelect]
        KeyInteract = []
        SelectionWidth = 3
        SelectionColor = clRed
        Style = iemsFlat
        AutoAdjustStyle = False
        ThumbnailsBackground = clBlack
        ThumbnailsBackgroundSelected = clBtnFace
        ThumbnailsBorderWidth = 2
        ThumbnailsBorderColor = clSilver
        MultiSelectionOptions = []
        DefaultTopText = iedtNone
        DefaultInfoText = iedtNone
        DefaultBottomText = iedtNone
        Align = alClient
        TabOrder = 0
        OnDblClick = ImageEnMViewDblClick
      end
    end
  end
  object q_compare_layer: TUniQuery
    UpdatingTable = 'CUST_COMPARE_LAYER'
    SQLInsert.Strings = (
      'INSERT INTO CUST_COMPARE_LAYER'
      '  (ID, MEMBER_UID, MAKE_DATE, COMPARE_NAME, COMPANY_ID)'
      'VALUES'
      '  (:ID, :MEMBER_UID, :MAKE_DATE, :COMPARE_NAME, :COMPANY_ID)')
    SQLDelete.Strings = (
      'DELETE FROM CUST_COMPARE_LAYER'
      'WHERE'
      '  ID = :Old_ID')
    SQLUpdate.Strings = (
      'UPDATE CUST_COMPARE_LAYER'
      'SET'
      
        '  ID = :ID, MEMBER_UID = :MEMBER_UID, MAKE_DATE = :MAKE_DATE, CO' +
        'MPARE_NAME = :COMPARE_NAME, COMPANY_ID = :COMPANY_ID'
      'WHERE'
      '  ID = :Old_ID')
    SQLLock.Strings = (
      'SELECT NULL FROM CUST_COMPARE_LAYER'
      'WHERE'
      'ID = :Old_ID'
      'FOR UPDATE WITH LOCK')
    SQLRefresh.Strings = (
      
        'SELECT ID, MEMBER_UID, MAKE_DATE, COMPARE_NAME, COMPANY_ID FROM ' +
        'CUST_COMPARE_LAYER'
      'WHERE'
      '  ID = :ID')
    Connection = dmDBCommon.UniDBConnection
    SQL.Strings = (
      'select * from cust_compare_layer'
      'where member_uid = :member_uid')
    SpecificOptions.Strings = (
      'InterBase.AutoClose=True'
      'InterBase.DeferredBlobRead=True'
      'InterBase.StreamedBlobs=True')
    Left = 624
    Top = 512
    ParamData = <
      item
        DataType = ftString
        Name = 'member_uid'
        ParamType = ptInput
      end>
    object q_compare_layerID: TStringField
      FieldName = 'ID'
      Required = True
      Size = 17
    end
    object q_compare_layerMEMBER_UID: TStringField
      FieldName = 'MEMBER_UID'
      Size = 17
    end
    object q_compare_layerMAKE_DATE: TDateField
      FieldName = 'MAKE_DATE'
    end
    object q_compare_layerCOMPARE_NAME: TStringField
      FieldName = 'COMPARE_NAME'
      Size = 30
    end
    object q_compare_layerCOMPANY_ID: TStringField
      FieldName = 'COMPANY_ID'
      Size = 17
    end
  end
  object d_compare_layer: TDataSource
    DataSet = q_compare_layer
    Left = 624
    Top = 560
  end
  object ActionThumbnail: TActionList
    Images = ImageListThumbnail
    Left = 496
    Top = 552
    object actThumbWindowMax: TAction
      Caption = #50040#45348#51068#52285' '#53356#44172
      ImageIndex = 1
      ShortCut = 121
    end
    object actThumbWindowMin: TAction
      Caption = #50040#45348#51060#52285' '#51089#44172
      ImageIndex = 0
      ShortCut = 120
    end
    object actThumbDelete: TAction
      Caption = #50040#45348#51068' '#49325#51228
      ImageIndex = 2
      ShortCut = 16430
      OnExecute = actThumbDeleteExecute
    end
    object actThumbSaveAs: TAction
      Caption = #50040#45348#51068' '#51200#51109
      ImageIndex = 3
      ShortCut = 16467
      OnExecute = actThumbSaveAsExecute
    end
    object ActAddLayerSet: TAction
      Caption = #49464#53944#52628#44032
      ImageIndex = 4
      OnExecute = ActAddLayerSetExecute
    end
    object ActSaveLayerSet: TAction
      Caption = #49464#53944#48320#44221#51200#51109
      ImageIndex = 5
      OnExecute = ActSaveLayerSetExecute
    end
    object ActDeleteLayerSet: TAction
      Caption = #49464#53944#49325#51228
      ImageIndex = 6
      OnExecute = ActDeleteLayerSetExecute
    end
    object ActRotationMode: TAction
      AutoCheck = True
      Caption = #54924#51204#47784#46300
      OnExecute = ActRotationModeExecute
    end
    object ActDeleteSelect: TAction
      Hint = #49440#53469#49325#51228
      OnExecute = ActDeleteSelectExecute
    end
    object ActDeleteAll: TAction
      Hint = #51204#52404#49325#51228
      OnExecute = ActDeleteAllExecute
    end
    object ActLayerToUp: TAction
      Hint = #50948#47196
      OnExecute = ActLayerToUpExecute
    end
    object ActLayerToDown: TAction
      Hint = #50500#47000#47196
      OnExecute = ActLayerToDownExecute
    end
    object ActLayerAutofit: TAction
      AutoCheck = True
      Caption = #51088#46041#47582#52644
      Checked = True
      OnExecute = ActLayerAutofitExecute
    end
    object ActLayerOutline: TAction
      AutoCheck = True
      Caption = #50980#44285#49440
      Checked = True
      OnExecute = ActLayerOutlineExecute
    end
    object ActEffectLayer: TAction
      AutoCheck = True
      Caption = #47112#51060#50612
      Checked = True
      OnExecute = ActEffectLayerExecute
    end
    object ActSaveSetup: TAction
      Caption = #49444#51221#44050#51200#51109
    end
  end
  object ImageListThumbnail: TImageList
    Left = 496
    Top = 600
    Bitmap = {
      
      000000000000}
  end
  object SaveImageEnDialog1: TSaveImageEnDialog
    Left = 496
    Top = 648
  end
  object PopupLayer: TPopupMenu
    AutoHotkeys = maManual
    Images = ImageListThumbnail
    Left = 848
    Top = 328
    object N5: TMenuItem
      Action = ActAddLayerSet
    end
    object N6: TMenuItem
      Action = ActSaveLayerSet
    end
    object N7: TMenuItem
      Action = ActDeleteLayerSet
    end
  end
  object PopupMenu1: TPopupMenu
    AutoHotkeys = maManual
    Left = 656
    Top = 32
    object N8: TMenuItem
      Action = ActRotationMode
      AutoCheck = True
    end
    object N9: TMenuItem
      Action = ActDeleteSelect
      Caption = #49440#53469#49325#51228
    end
    object N10: TMenuItem
      Action = ActDeleteAll
      Caption = #51204#52404#49325#51228
    end
    object N11: TMenuItem
      Action = ActLayerToUp
      Caption = #50948#47196
    end
    object N12: TMenuItem
      Action = ActLayerToDown
      Caption = #50500#47000#47196
    end
    object N17: TMenuItem
      Caption = '-'
    end
    object N13: TMenuItem
      Action = ActLayerAutofit
      AutoCheck = True
    end
    object N14: TMenuItem
      Action = ActLayerOutline
      AutoCheck = True
    end
    object N15: TMenuItem
      Action = ActEffectLayer
      AutoCheck = True
    end
    object N18: TMenuItem
      Caption = '-'
    end
    object N16: TMenuItem
      Action = ActSaveSetup
    end
  end
  object q_compare_layer_image: TUniQuery
    SQLInsert.Strings = (
      'INSERT INTO CUST_COMPARE_LAYER_IMAGE'
      '  (ID, MEMBER_UID, LAYER_DATA)'
      'VALUES'
      '  (:ID, :MEMBER_UID, :LAYER_DATA)')
    SQLDelete.Strings = (
      'DELETE FROM CUST_COMPARE_LAYER_IMAGE'
      'WHERE'
      '  ID = :Old_ID')
    SQLUpdate.Strings = (
      'UPDATE CUST_COMPARE_LAYER_IMAGE'
      'SET'
      '  ID = :ID, MEMBER_UID = :MEMBER_UID, LAYER_DATA = :LAYER_DATA'
      'WHERE'
      '  ID = :Old_ID')
    SQLLock.Strings = (
      'SELECT NULL FROM CUST_COMPARE_LAYER_IMAGE'
      'WHERE'
      'ID = :Old_ID'
      'FOR UPDATE WITH LOCK')
    SQLRefresh.Strings = (
      'SELECT ID, MEMBER_UID, LAYER_DATA FROM CUST_COMPARE_LAYER_IMAGE'
      'WHERE'
      '  ID = :ID')
    Connection = dmDBCommon.UniDBConnection
    SQL.Strings = (
      'select * from cust_compare_layer_image'
      'where member_uid = :member_uid')
    Left = 760
    Top = 512
    ParamData = <
      item
        DataType = ftString
        Name = 'member_uid'
        ParamType = ptInput
      end>
    object q_compare_layer_imageID: TStringField
      FieldName = 'ID'
      Required = True
      Size = 17
    end
    object q_compare_layer_imageMEMBER_UID: TStringField
      FieldName = 'MEMBER_UID'
      Size = 17
    end
    object q_compare_layer_imageLAYER_DATA: TBlobField
      FieldName = 'LAYER_DATA'
    end
  end
  object d_compare_layer_image: TDataSource
    DataSet = q_compare_layer_image
    Left = 760
    Top = 560
  end
end
